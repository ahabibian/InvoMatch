...
def test_reconcile_results_are_bound_to_invoice_ids():
    ...
    assert all(
        result.match_result.status in {"matched", "duplicate_detected", "partial_match", "unmatched"}
...
def test_reconcile_and_save_marks_run_failed_when_execution_raises(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(
        ReconciliationExecutionError,
        match="Reconciliation execution failed: reconciliation exploded",
    ):
        reconcile_and_save(
            ROOT_DIR / "sample-data" / "invoices.csv",
            ROOT_DIR / "sample-data" / "payments.csv",
    ...
    failed_run = load_reconciliation_run(runs[0].run_id, run_store=run_store)

    assert failed_run.status == "failed"
    assert failed_run.started_at is not None
    assert failed_run.finished_at is not None
    assert failed_run.error is not None
    assert failed_run.error.code == "execution_failure"
    assert failed_run.error.message == "reconciliation exploded"
...
def test_reconcile_and_save_rejects_missing_invoice_file(tmp_path: Path):
    ...
    with pytest.raises(
        ReconciliationInputValidationError,
        match="invoice_csv_path does not exist",
    ):
        reconcile_and_save(
...
def test_reconcile_and_save_rejects_wrong_extension(tmp_path: Path):
    ...
    with pytest.raises(
        ReconciliationInputValidationError,
        match="invoice_csv_path must point to a .csv file",
    ):
        reconcile_and_save(
            invoice_path,
            ROOT_DIR / "sample-data" / "payments.csv",
