...
def test_repository_saves_and_finds_latest_by_identity_key():
    repository = InMemoryIngestionRepository()
    result = ingest_invoice_input(
        RawInvoiceInput(
            invoice_number="INV-001",
    ...
    saved = repository.save_result(result)
    found = repository.find_latest_by_identity_key(result.identity_key)

    assert found is not None
    assert found.record_id == saved.record_id
    assert found.result.identity_key == result.identity_key
...
def test_repository_returns_none_for_unknown_identity_key():
    repository = InMemoryIngestionRepository()
    assert repository.find_latest_by_identity_key("missing") is None